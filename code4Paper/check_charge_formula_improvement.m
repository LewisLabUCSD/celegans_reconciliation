...
formula_list = printRxnFormula(model,'rxnAbbrList',rxns,'printFlag',false,'directionFlag',true,'gprFlag',true);
% formula_list
...
for i=1:length(met_list)
    old_charge(i,1) = model.metCharges(strcmp(model.mets,met_list{i,1}));
    new_charge(i,1) = prop_mets.charge{i,1};
    model1.metCharges(strcmp(model.mets,met_list{i,1})) = int32(prop_mets.charge{i,1});
end
